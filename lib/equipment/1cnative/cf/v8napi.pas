...
// 18.02.2018 stoi: Расширена функция _V8PGS. Добавлен параметр PropName
// 25.02.2019 stoi: Добавлен расширенный интерфейс 1С, другие доработки
// 10.10.2019 antara: Доработка под x64
...
type
  ...
  // IAddInDefBaseEx см. документацию от 1С
...
type
  ...
  public
    ...
    // можно override вызывается из v8wrap
...
// целое или вещественное число, код ошибки
...
// ДатаВремя (VTYPE_DATE или VTYPE_TM)
...
// тип Double возвращается как целая часть
function V8AsDouble(V: PV8Variant): double;
// тип integer возвращается как Double
...
begin
  try
    ...
    begin
      ...
      Obj.RelObj1.V8.AddError(Ord(ADDIN_E_FAIL), 'Внешняя компонента',
