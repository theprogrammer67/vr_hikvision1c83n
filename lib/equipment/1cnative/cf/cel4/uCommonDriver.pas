/// /////////////////////////////////////////////////////////////////////////////
//
// Базовый класс драйвера ТО для ревизии интерфейса (версии требований 1С) 4.0
// Драйверы 1С:БПО
...
uses uCustomDriver, v8napi, System.SysUtils, Xml.XMLDoc, Xml.XMLIntf,
  System.Classes, Winapi.Windows, System.Rtti, System.Variants,
  System.Generics.Collections, uMarkdownRenderer;
...
{$IFDEF Use_DeviceId}
  ...
  TSettings = TDictionary<WideString, Variant>;
{$ENDIF}
  ...
  public type
    TValue = (etScanner, etMCR, etFR, etReceiptPrinter, etLabelPrinter,
      ...
      etPrintingScale, etRFIDReader, etECR, etVideocamera, etElectronicLock,
      etUnknown);
  protected
...
{$ENDIF}
  ...
  protected
{$IFDEF Use_DeviceId}
    procedure TryReopenWithDeviceID(const ADeviceID: string);
{$ENDIF}
    ...
    function GetLocalizationPattern: string; overload; virtual;
    procedure SetLocalization(const ALanguageCode, ALocalizationPattern
  ...
  public
    ...
    function DoAdditionalAction(RetValue: PV8Variant; Params: PV8ParamArray;
      const ParamCount: Integer; var v8: TV8AddInDefBase): Boolean; overload;
    [ExportMethAttribute('ПолучитьШаблонЛокализации', 1)]
    function GetLocalizationPattern(RetValue: PV8Variant; Params: PV8ParamArray;
      const ParamCount: Integer; var v8: TV8AddInDefBase): Boolean; overload;
    [ExportMethAttribute('УстановитьЛокализацию ', 2)]
    function SetLocalization(RetValue: PV8Variant; Params: PV8ParamArray;
      const ParamCount: Integer; var v8: TV8AddInDefBase): Boolean; overload;
  public
...
begin
{$IFDEF Use_DeviceId}
  if (ADeviceID <> '') and (not FListSettings.ContainsKey(ADeviceID)) then
    raise Exception.Create
      (Format('Экземпляр устройства c идентификатором %s не существует или уже был отключен',
      [ADeviceID]))
  else if (ADeviceID = '') and (not Enabled) then
...
function TCommonDriver.GetLocalizationPattern: string;
begin
  Result := '';
end;

function TCommonDriver.GetLocalizationPattern(RetValue: PV8Variant;
  Params: PV8ParamArray; const ParamCount: Integer;
  var v8: TV8AddInDefBase): Boolean;
begin
  try
    V8SetWString(@Params[1], GetLocalizationPattern);
    SetSuccess(RetValue);
  except
    SetError(ExceptObject, RetValue);
  end;
...
begin
  try
    Open;
{$IFDEF Use_DeviceId}
    FCurrentDeviceID :=  Copy(TGUID.NewGuid.ToString, 2, 36);
    LSettings := TSettings.Create();
    TDictionaryHelpers<WideString, Variant>.CopyDictionary(FSettings,
      LSettings);
    FListSettings.AddOrSetValue(FCurrentDeviceID, LSettings);
    V8SetWString(@Params[1], FCurrentDeviceID);
    FSettings.Clear;
{$ENDIF}
    SetSuccess(RetValue);
  except
...
procedure TCommonDriver.SetLocalization(const ALanguageCode,
  ALocalizationPattern: string);
begin

end;

function TCommonDriver.SetLocalization(RetValue: PV8Variant;
  Params: PV8ParamArray; const ParamCount: Integer;
  var v8: TV8AddInDefBase): Boolean;
begin
  try
    SetLocalization(V8AsWString(@Params[1]), V8AsWString(@Params[2]));
    SetSuccess(RetValue);
  except
    SetError(ExceptObject, RetValue);
  end;
...
procedure TCommonDriver.TryReopenWithDeviceID(const ADeviceID: string);
var
  LSettings: TSettings;
  LKey: string;
begin
  if (ADeviceID <> '') and (FCurrentDeviceID <> ADeviceID) then
  begin
    if not FListSettings.ContainsKey(ADeviceID) then
      raise Exception.Create
        (Format('Экземпляр устройства c идентификатором %s не существует или уже был отключен',
        [ADeviceID]));
