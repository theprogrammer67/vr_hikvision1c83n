...
uses System.SysUtils, uCommonDriver, uCustomDriver, v8napi;
...
type
  ...
  protected
    ...
    procedure MakePhoto(const AFileName: string; out AImage: TBytes); overload;
      virtual; abstract;
    // Получение снимка в виде двоичных данных
    procedure GetPhoto(out AImage: TBytes); overload; virtual; abstract;
  public
    ...
    function StopVideo(RetValue: PV8Variant; Params: PV8ParamArray;
      const ParamCount: Integer; var v8: TV8AddInDefBase): Boolean; overload;
    // Захват видео, создание снимка и запись в файл
    [ExportMethAttribute('СделатьФотоснимок', 2)]
    function MakePhoto(RetValue: PV8Variant; Params: PV8ParamArray;
      const ParamCount: Integer; var v8: TV8AddInDefBase): Boolean; overload;
    // Получение снимка в виде двоичных данных
    [ExportMethAttribute('ПолучитьФотоснимок', 1)]
    function GetPhoto(RetValue: PV8Variant; Params: PV8ParamArray;
      const ParamCount: Integer; var v8: TV8AddInDefBase): Boolean; overload;
  public
...
function TVideocameraDriver.GetPhoto(RetValue: PV8Variant;
...
begin
  try
    ...
    if Length(LImage) > 0 then
      V8SetBlob(@Params[1], @LImage[0], Length(LImage));
...
begin
  try
    ...
    if LFileName.IsEmpty then
      LFileName := PhotoFileName;

    SetLength(LImage, 0);
    MakePhoto(LFileName, LImage);
    if Length(LImage) > 0 then
      V8SetBlob(@Params[2], @LImage[0], Length(LImage));
