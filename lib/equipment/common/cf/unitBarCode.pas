...
type
  ...
  public
    ...
    function GetBarCodeWidth(ACanvas: TCanvas; InPixels: Boolean = False): Double;
    ...
    property DPIX: Integer read GetDPIX write FDPI; // stoi
    property DPIY: Integer read GetDPIY write FDPI; // stoi
    property Alignment: TAlignment read FAlignment write SetAlignment; // stoi
//    procedure CopyToBuffer(Sender: TObject); override;
//	procedure ConvertToString(SL: TStringList); override;
...
var
  ...
  procedure DrawErrorText;
  begin
    TR := R;
    Inc(TR.Left, 4);
    Canvas.Brush.Style := bsClear;
    DrawText(Canvas.Handle, 'ОШИБКА!', 7, TR, DT_VCENTER or DT_CENTER);
...
begin
  ...
  PixelsPerInchX := DPIX; // stoi

  if FDataError then begin
    DrawErrorText;
    Exit;
  end;
  ...
  {total width of BarCode and position within rect}
    ...
    if (BarCodeWidth < RectWidth(R)) and (Alignment <> taLeftJustify) then // stoi
...
function TStBarCode.GetBarCodeWidth(ACanvas: TCanvas; InPixels: Boolean = False): Double;
var
  PixelsPerInchX: Integer;
  SmallestWidth: Double;
begin
//  PixelsPerInchX := GetDeviceCaps(ACanvas.Handle, LOGPIXELSX);
  ...
  if not InPixels then {return width of barcode in inches}
...
begin
  ...
  case FBarCodeType of
    bcUPC_A, bcUPC_E, bcEAN_8, bcEAN_13, bcInterleaved2of5:
      begin
        Result := Length(Trim(Characters));
        if Result = 0 then
          raise Exception.Create('Штрихкод имеет нулевую длину');
        try
          for I := 1 to Result do
            bcDigits[I] := StrToInt(Characters[I]);
        except
          raise Exception.Create('Штрихкод содержит некорректный символ');
        end;
      end;
    bcCodabar:
      begin
      ...
      begin
        ...
        for I := 1 to Result do begin
          case Characters[I] of
            ...
            '}': bcDigits[I] := 93;
            '~': bcDigits[I] := 94;
            #130: bcDigits[I] := 98; {й} {rest are manufactured characters}
            #131: bcDigits[I] := 97; {в}
            #132: bcDigits[I] := 96; {д}
            #133: bcDigits[I] := 98; {а}
            #134: bcDigits[I] := 100; {е}
            #135: bcDigits[I] := 99; {з}
            #136: bcDigits[I] := 103; {к}
            #137: bcDigits[I] := 104; {л}
            #138: bcDigits[I] := 105; {и}
            #139: bcDigits[I] := 106; {п}
          else
            raise Exception.Create('Штрихкод содержит некорректный символ');
            // RaiseStError(EStBarCodeError, stscInvalidCharacter);
          end;
...
function TStBarCode.GetDPIX: Integer;
begin
  if FDPI = 0 then // stoi
...
function TStBarCode.GetDPIY: Integer;
begin
  if FDPI = 0 then // stoi
...
procedure TStBarCode.SetAlignment(Value: TAlignment); // stoi
begin
      ...
      bcUPC_A:
        begin
          ...
          if not (Length(Code) in [11, 12]) then
            raise Exception.Create('Invalid UPCA Code Len');
...
//            RaiseStError(EStBarCodeError, stscInvalidCharacter);
          ...
          if (Length(Code) = 12) and (CheckC <> bcDigits[12]) then
            raise Exception.Create('Неверный контрольный символ в штрихкоде');
      ...
      bcEAN_8:
        begin
          ...
          if not (Length(Code) in [7, 8]) then
            raise Exception.Create('Invalid UPCA Code Len');
//            raise Exception.Create( stscInvalidEAN8CodeLen);
          try
            GetDigits(Code);
          except
            raise Exception.Create('Штрихкод содержит некорректный символ');
//            raise Exception.Create( stscInvalidCharacter);
          end;
      ...
      bcEAN_13:
        begin
          ...
          if not (Length(Code) in [12, 13]) then
            raise Exception.Create('Invalid EAN13 Code Len');
          try
            GetDigits(Code);
          except
            raise Exception.Create('Штрихкод содержит некорректный символ');
    ...
    if FSupplementalCode > '' then
      if not (Length(FSupplementalCode) in [2, 5]) then
        raise Exception.Create('Invalid Sup Code Len');
  except
    Result := False;
...
{procedure TStBarCode.CopyToBuffer(Sender: TObject);
begin
  inherited;
    BufferType := OT_BARCODE;
    Buffer.Text := Code;

end;}

{procedure TStBarCode.ConvertToString(SL: TStringList);
begin
    inherited;
    SL.Add('TYPE=CODE');
    SL.Add('Data='+Code);
    SL.Add('Kind='+IntToStr(Ord(BarCodeType)));

end;}

procedure TStBarCode.ShowProperties(Sender: TObject);
begin
  frmBarCodeProp := TfrBarCodeProp.Create(nil);
  with frmBarCodeProp do begin
    cbType.ItemIndex := Ord(BarCodeType);
    cbCode.Text := Code;
    edBarNarrowToWideRatio.Text := IntToStr(BarNarrowToWideRatio);
    edBarToSpaceRatio.Text := FloatToStr(BarToSpaceRatio);
    edBarWidth.Text := FloatToStr(BarWidth);
    chbBearerBars.Checked := BearerBars;
    cbCode128Subset.ItemIndex := Ord(Code128Subset);
    chbShowCode.Checked := ShowCode;
    chbShowGuardChars.Checked := ShowGuardChars;
    edSupplementalCode.Text := SupplementalCode;
    chbTallGuardBars.Checked := TallGuardBars;
    chbAddCheckChar.Checked := AddCheckChar;
    if ShowModal = mrOk then begin
      BarCodeType := TStBarCodeType(cbType.ItemIndex);
      Code := cbCode.Text;
      BarNarrowToWideRatio := StrToInt(edBarNarrowToWideRatio.Text);
      BarToSpaceRatio := StrToFloat(edBarToSpaceRatio.Text);
      BarWidth := StrToFloat(edBarWidth.Text);
      BearerBars := chbBearerBars.Checked;
      Code128Subset := TStCode128CodeSubset(cbCode128Subset.ItemIndex);
      ShowCode := chbShowCode.Checked;
      ShowGuardChars := chbShowGuardChars.Checked;
      SupplementalCode := edSupplementalCode.Text;
      TallGuardBars := chbTallGuardBars.Checked;
      AddCheckChar := chbAddCheckChar.Checked;
    end;
  end;
...
{procedure TStBarCode.SetProperties(Sender: TObject);
begin
  inherited;
  if frmPropFrame = nil then Exit;
  if frmPropFrame is TfrBarCodeProp then
 with frmPropFrame as TfrBarCodeProp do begin
  BarCodeType := TStBarCodeType(cbType.ItemIndex);
  Code := cbCode.Text;
  BarNarrowToWideRatio := StrToInt(edBarNarrowToWideRatio.Text);
  BarToSpaceRatio := StrToFloat(edBarToSpaceRatio.Text);
...
procedure Register;
begin
  RegisterComponents('Additional', [TStBarCode]);
end;

initialization
  RegisterClass(TStBarCode);
end.
